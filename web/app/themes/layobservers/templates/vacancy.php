<div <?php post_class('card card-default'); ?> id="job-<?= get_the_ID(); ?>">
  <div class="card-heading" role="tab" id="heading<?php the_ID(); ?>">
    <h3 class="card-title entry-title">
      <a class="collapsed" role="button" data-toggle="collapse" href="#collapse<?php the_ID(); ?>" aria-expanded="false"
         aria-controls="collapse<?php the_ID(); ?>">
        <?php the_title(); ?>
      </a>
    </h3>
  </div>
  <div id="collapse<?php the_ID(); ?>" class="card-collapse collapse" aria-labelledby="heading<?php the_ID(); ?>"
    ...
    <div class="card-body">
      ...
      <?php if (!empty($rcr)): ?>
        <p><strong>Recruitment Campaign Reference (to be quoted on all correspondence and forms):</strong> <?= $rcr ?>
        </p>
      <?php endif; ?>
